...
router.get("/auth/saml/:provider/callback", validateProvider, processLogout);
...
router.get(
  "/auth/:provider/:token/:authParams",
  validateProvider,
  validateToken,
  parseParams,
  authenticateRequest
...
router.get("/auth/meta/idp/:idp", function (req, res) {
  ...
  // handle ENOENT
  fs.readFile(safeFileFullPath, (err, data) => {
    if (err) {
...
router.get("/logout/request/:authParams", parseParams, (req, res, next) => {
  ...
  if (strategy.name === "saml" && strategy._saml.options.logoutUrl) {
    ...
    appInsights.defaultClient.trackEvent({
      name: "SP-initiated Logout Request",
...
router.get("/logout/response/:status?", (req, res, next) => {
  ...
  if (
    !(req.user && req.user.logoutRequest && req.user.logoutRequest.provider)
  ) {
    ...
    req.samlLogoutRequest.status =
      "urn:oasis:names:tc:SAML:2.0:status:" + status;
    strategy._saml.getLogoutResponseUrl(
      req,
      req.user.relayState,
      {},
      (err, url) => {
        if (err) {
          ...
          appInsights.defaultClient.trackEvent({
            name: "IDP-Initiated Logout Response",
            properties: req.samlLogoutRequest,
          });
          res.redirect(url);
        }
      }
...
function validateProvider(req, res, next) {
  ...
  if (providerConfData) {
    ...
    webutil.handleError(
      req,
      res,
      "The selected provider is not recognized as external identity provider"
    );
  }
}
...
async function authenticateRequest(req, res, next) {
  ...
  appInsights.defaultClient.trackEvent({
    ...
    properties: { ...{ provider: provider_id }, ...req.query, ...req.session },
  });

  const providerConfData = global.providers.find(
    (cfg) => cfg.id === provider_id
  );
  ...
  if (providerConfData.type === "openid-client") {
    ...
    if (
      client.use_request_object &&
      client.use_request_object.toString().toLowerCase() === "true"
    ) {
      // Only for clients with use_request_object set to true
  ...
  passport.authenticate(req.params.provider, req.passportAuthenticateParams)(
    req,
    res,
    next
  );
...
function authenticateRequestCasa(req, res, next) {
  ...
  logger.log2(
    "verbose",
    `Proceeding with linking procedure for provider ${provider}`
  );
...
function authenticateRequestCallback(req, res, next) {
  logger.log2(
    "verbose",
    `Authenticating request against ${req.params.provider}`
  );
  passport.authenticate(req.params.provider, {
    failureRedirect: "/passport/error",
    failureFlash: true,
  })(req, res, next);
}
...
function processLogout(req, res) {
  const validateCallback = async ({ profile, loggedOut }) => {
    logger.log2(
      "debug",
      "logout callback " + JSON.stringify(profile) + " " + loggedOut
    );

    if (profile) {
      // received a Logout Request
      appInsights.defaultClient.trackEvent({
        name: "IDP-Initiated Logout Request",
        properties: profile,
      });

      req.samlLogoutRequest = profile;
      if (req.session && req.session.authenticating) {
        // Login is in flight. Must be Administrative SLO.
        ...
        // Can't find oxAuth session cookie(s). Probably blocked.
        req.samlLogoutRequest.status =
          "urn:oasis:names:tc:SAML:2.0:status:Responder";
        req.samlLogoutRequest.reason = "Session not found";
      } else if (req.cookies.session_id.length === 0) {
        // Session no longer exists
        req.samlLogoutRequest.reason = "Old session";
      } else {
        // Check the oxAuth session_status API
        const responsePromise = got.get(
          global.basicConfig.sessionStatusEndpont,
          {
        ...
        const [httpResponse, statusResponse] = await Promise.all([
          responsePromise,
          jsonPromise,
        ]);
        ...
        if (
          ...
          statusResponse.state != "authenticated" ||
          isNaN(expiry) ||
          expiry - Date.now() < 5000
        ) {
          // Dont't risk it if session is about to expire
          req.samlLogoutRequest.reason = "Old session";
        }
      }

      if (req.samlLogoutRequest.reason) {
        // Respond immediately
        strategy._saml.getLogoutResponseUrl(
          req,
        ...
        // Propogate logout to oxAuth
        req.user.logoutRequest = profile;
        req.user.logoutRequest.provider = provider;
        req.user.relayState = req.query.RelayState;
        const redirectUri = encodeURIComponent(
          "https://" + req.hostname + "/passport/logout/response"
        );
        res.redirect(
          "/oxauth/restv1/end_session?post_logout_redirect_uri=" + redirectUri
        );
      }
    } else {
      // Sending Logout Response
      appInsights.defaultClient.trackEvent({
        name: "SP-Initiated Logout Response",
        properties: profile,
      });
      res.send("Success");
    }
...
function parseParams(req, res, next) {
  ...
  try {
    if (params) {
      ...
      const paramsObj = misc.decrypt(
        params.replace("_", "/").replace("-", "+")
      );
