...
const OPERATION_NO_CONN =
  "Attempt to operate on cache provider but connection has not been established";

const promisify = (context, methodName) =>
  Promise.promisify(context[methodName], { context: context });
...
function getRedisProvider(options, exp) {
  logger.log2(
    "info",
    "Configuring redis cache provider for inResponseTo validation"
  );
...
function getMemcachedProvider(options, exp) {
  logger.log2(
    "info",
    "Configuring memcached provider for inResponseTo validation"
  );
