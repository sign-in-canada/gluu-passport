...
function getTokenEndpoint(umaConfigURL) {
  ...
  return got
    .get(umaConfigURL, { responseType: "json" })
    .then((response) => {
      const body = response.body;
      logger.log2("debug", `getTokenEndpoint. obj = ${JSON.stringify(body)}`);
...
function getRPT(ticket, tokenEndpoint) {
  ...
  logger.log2(
    "debug",
    `getRPT request to ${tokenEndpoint} endpoint with options = ${JSON.stringify(
      options,
      null,
      4
    )}`
  ...
  return got
    .post(tokenEndpoint, options)
    .then((response) => {
      const rptDetails = response.body;
      logger.log2(
        "debug",
        `getRPT. response: ${JSON.stringify(rptDetails, null, 4)}`
      );
...
function doRequest(requestOptions) {
  ...
  logger.log2(
    "debug",
    `doRequest. options = ${JSON.stringify(options, null, 4)}`
  );
  return got.get(uri, options).then((response) => {
    ...
    switch (statusCode) {
      ...
      case 200: {
        ...
        logger.log2("debug", `getConfiguration. Passport configs are: ${body}`);
        // return this as val to function(val) inside request
        return JSON.parse(body);
      }
      default: {
        throw new Error(
          "Received unexpected HTTP status code " + `of ${statusCode}`
        );
      }
    }
  });
...
function request(options) {
  ...
  const chain = misc.pipePromise(doRequest, fn);
