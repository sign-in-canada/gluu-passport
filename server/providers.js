...
function applyMapping(profile, provider) {
  ...
  try {
    const mapping = global.providers.find(
      (providerObj) => providerObj.id === provider
    ).mapping;
...
async function setupStrategy(provider) {
  logger.log2(
    "info",
    `Setting up strategy for provider ${provider.displayName}`
  );
  logger.log2(
    "debug",
  ...
  if (isSaml) {
    ...
    if (
      global.iiconfig.authorizationParams.find(
    ...
    if (providerOptions.validateInResponseTo) {
      ...
      if (!f(providerOptions.redisCacheOptions)) {
        providerOptions.cacheProvider = cacheProvider.get(
          "redis",
          providerOptions.redisCacheOptions,
          exp
        );
      } else if (!f(providerOptions.memcachedCacheOptions)) {
        providerOptions.cacheProvider = cacheProvider.get(
          "memcached",
          providerOptions.memcachedCacheOptions,
          exp
        );
      }
    }

    const samlStrategy = new Strategy(providerOptions, (req, profile, cb) => {
...
function fixDataTypes(providers) {
  for (const provider of providers) {
    ...
    if (misc.isObject(value)) {
      ...
      logger.log2(
        "warn",
        `Object expected for property options, found ${JSON.stringify(value)}`
      );
      value = {};
    }
    ...
    if (!misc.isObject(value)) {
      ...
      if (!R.isNil(value)) {
        logger.log2(
          "warn",
          `Parsable object expected for property passportAuthnParams, found ${JSON.stringify(
            value
          )}`
        );
      }
...
function fillMissingData(providers) {
  ...
  for (const provider of providers) {
    ...
    if (isSaml) {
      ...
      options.callbackUrl = R.defaultTo(
        `${prefix}/saml/${provider.id}/callback`,
        callbackUrl
      );
    } else if (strategyId === "openid-client") {
      options.redirect_uris = [
        R.defaultTo(`${prefix}/${provider.id}/callback`, callbackUrl),
      ];
    } else {
      options.callbackURL = R.defaultTo(
        `${prefix}/${provider.id}/callback`,
        callbackUrl
      );
    ...
    if (strategyId.indexOf("passport-apple") >= 0 && options.key) {
      ...
      try {
        ...
        logger.log2(
          "warn",
          `There was a problem reading file ${options.key}. Ensure the file exists and is readable`
        );
...
function setup(providers) {
  ...
  if (hashConfig !== prevConfigHash) {
    ...
    R.forEach(
      (s) => passport.unuse(s),
      R.map(R.prop("id"), passportStrategies)
    );
